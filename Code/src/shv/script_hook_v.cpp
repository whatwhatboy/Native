...
NativeContext2 ScriptHookV::Context;
...
void ScriptHookV::scriptRegister(HMODULE module, void(*LP_SCRIPT_MAIN)()) {
	...
	if (GetModuleFileNameA(module, name, 256)) {
		if (strstr(name, "Menyoo")) {
			...
			LOG(Info, "Registered nops & menyoo");
		}
	}
	...
	Base::g_fiber_manager.add(va("0x%llx:0x%llx", (uint64_t)module, (uint64_t)LP_SCRIPT_MAIN), LP_SCRIPT_MAIN);
	LOG(Info, "Registered fiber 1");
}

void ScriptHookV::scriptRegisterAdditionalThread(HMODULE module, void(*LP_SCRIPT_MAIN)()) {
	//GetScriptHookFiberManager()->AddFiber(va("0x%llx:0x%llx", (uint64_t)module, (uint64_t)LP_SCRIPT_MAIN), LP_SCRIPT_MAIN);
	Base::g_fiber_manager.add(va("0x%llx:0x%llx", (uint64_t)module, (uint64_t)LP_SCRIPT_MAIN), LP_SCRIPT_MAIN);
	LOG(Info, "Registered fiber 2");
}
...
uint64_t* ScriptHookV::nativeCall() {
	...
	if (Hash == 0x509D5878EB39E842 || Hash == 0x9A294B2138ABB884 || Hash == 0x7DD959874C1FD534)
		Context.SetArg(4, true);

	if (Hash == 0x509D5878EB39E842) {
		Context.SetArg(5, true);
		Context.SetArg(6, false);
	}
	...
	if (Hash == 0xD49F9B0955C367DE) {
		Context.SetArg(6, true);
		Context.SetArg(7, false);
	}
	...
	if (!found) {
		Base::g_invoker->Call(Hash, &Context);
		LOG(Info, "Call CTX Done");
	}
	...
	if (Hash == 0xD24D37CC275948CC) {
		Context.SetReturn(0, MISC::GET_HASH_KEY(Context.Arg<char*>(0)));
	}
	LOG(Info, "Natives completed");
	return reinterpret_cast<uint64_t*>(Context.GetResultPtr());
}

uint64_t* ScriptHookV::getGlobalPtr(int index) {
	if (index == 0x140815) index = (17411 + 4769);
	return (uint64_t*)&Base::g_pointers->m_global_base[index >> 18 & 0x3F][index & 0x3FFFF];
}
...
void ScriptHookV::scriptWait(unsigned long waitTime) {
	auto time{ std::chrono::milliseconds(waitTime) };
	Base::fiber::current()->wait(time);
}
...
#endif
	auto address = Base::g_utils->get_entity_address(handle);
...
int ScriptHookV::worldGetAllVehicles(int* arr, int arrSize) {
	auto vehicles = Base::g_pools->vehicles();
...
int ScriptHookV::worldGetAllObjects(int* arr, int arrSize) {
	auto objects = Base::g_pools->objects();
