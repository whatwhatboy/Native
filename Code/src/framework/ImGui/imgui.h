...
namespace ImGui
{
    ...
    IMGUI_API ImGuiID       get_id(const char* str_id);                                      // calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
    IMGUI_API ImGuiID       get_id(const char* str_id_begin, const char* str_id_end);
    IMGUI_API ImGuiID       get_id(const void* ptr_id);
    ...
    IMGUI_API ImVec2        get_mouse_pos();                                                      // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
    IMGUI_API ImVec2        get_mouse_posOnOpeningCurrentPopup();                                 // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
