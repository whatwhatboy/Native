...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->get_id(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImGuiID id = window->get_id(str_id);
...
{
    return window->get_id(axis == ImGuiAxis_X ? "#SCROLLX" : "#SCROLLY");
}
...
bool ImGui::ImageButton(const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    return ImageButtonEx(window->get_id(str_id), user_texture_id, size, uv0, uv1, bg_col, tint_col);
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImGuiID id = window->get_id("#image");
    PopID();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 check_col2 = ImColor(45, 45, 45, 255);
    ImU32 check_col3 = ImColor(60, 60, 60, 255);
    ImU32 check_col4 = ImColor(86, 87, 87, 255);
    if (mixed_value)
    {
    ...
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, check_col4, style.FrameRounding);

        window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImU32(ImColor(100, 100, 100, 255)), style.FrameRounding, 0, 0.1);
    }
    if (!*v)
    {
        //const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, check_col2, style.FrameRounding);
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, check_col3, style.FrameRounding, 0, 0.1);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 text_size = CalcTextSize(preview_value, NULL, false);
    const float text_width = text_size.x;
    const float combo_width = text_width + style.ItemInnerSpacing.x + arrow_size;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(combo_width, label_size.y + style.FramePadding.y * 2.0f));
    ...
    // Render shape
    const ImU32 frame_col = GetColorU32(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_Button);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        const ImVec2 preview_text_size = CalcTextSize(preview_value, NULL, false);
        const float preview_text_width = preview_text_size.x;
        const ImVec2 preview_text_pos = ImVec2(bb.Min.x + style.FramePadding.x, bb.Min.y + style.FramePadding.y);
        RenderTextClipped(preview_text_pos, ImVec2(preview_text_pos.x + preview_text_width, bb.Max.y), preview_value, NULL, &preview_text_size);
    }
    if (label_size.x > 0)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min + ImVec2(0, 3), frame_bb.Max - ImVec2(0, 3), GetColorU32(ImGuiCol_Button), true, 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min + ImVec2(3.5, 3), grab_bb.Max - ImVec2(3.5, 3), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 100);
    ...
    const bool is_grabbed = (g.ActiveId == id);
    if (is_grabbed)
    {
        char value_buf[64];
        const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
        ...
        ImVec2 value_text_pos = grab_bb.Min + ImVec2(24, 2);
        const float value_text_width = CalcTextSize(value_buf, value_buf_end).x;
        const float value_text_right = value_text_pos.x + value_text_width;

        // Adjust the position if the value text goes off the right side
        if (value_text_right > frame_bb.Max.x)
            value_text_pos.x = frame_bb.Max.x - value_text_width;

        window->DrawList->AddText(ImGui::GetFont(), ImGui::GetFontSize() - 1.00, value_text_pos, ImGui::GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
    }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline) // Open group before calling get_id() because groups tracks id created within their scope (including the scrollbar)
        BeginGroup();
    const ImGuiID id = window->get_id(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiID id = window->get_id(desc_id);
    const float default_size = GetFrameHeight();
...
bool ImGui::TreeNode(const char* label)
{
    ...
    if (window->SkipItems)
        return false;
    return TreeNodeBehavior(window->get_id(label), 0, label, NULL);
}
...
bool ImGui::TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags)
{
    ...
    return TreeNodeBehavior(window->get_id(label), flags, label, NULL);
}
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    return TreeNodeBehavior(window->get_id(str_id), flags, label, label_end);
}
...
bool ImGui::TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    const char* label, * label_end;
    ImFormatStringToTempBufferV(&label, &label_end, fmt, args);
    return TreeNodeBehavior(window->get_id(ptr_id), flags, label, label_end);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ImGuiDir_Right : ImGuiDir_Down, 1.1f);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y + g.FontSize * 0.15f), text_col, is_open ? ImGuiDir_Right : ImGuiDir_Down, 0.8f);
        if (g.LogEnabled)
...
bool ImGui::CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags)
{
    ...
    return TreeNodeBehavior(window->get_id(label), flags | ImGuiTreeNodeFlags_CollapsingHeader, label);
}
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        ImGuiID close_button_id = get_idWithSeed("#CLOSE", NULL, id);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    const ImGuiID close_button_id = p_open ? get_idWithSeed("#CLOSE", NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
