...
struct ImGuiContext
{
    ...
    ImGuiID                 DebugHookIdInfo;                    // Will call core hooks: DebugHookIdInfo() from get_id functions, used by Stack Tool [next HoveredId/ActiveId to not pull in an extra cache-line]
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiID                 MoveId;                             // == window->get_id("#MOVE")
...
public:
    ...
    ImGuiID     get_id(const char* str, const char* str_end = NULL);
    ImGuiID     get_id(const void* ptr);
    ImGuiID     get_id(int n);
    ImGuiID     get_idFromRectangle(const ImRect& r_abs);
...
//#endif
    ...
    // Basic Accessors
    inline ImGuiID          GetItemID()     { ImGuiContext& g = *GImGui; return g.LastItemData.ID; }   // Get ID of last item (~~ often same ImGui::get_id(label) beforehand)
    ...
    IMGUI_API ImGuiID       get_idWithSeed(const char* str_id_begin, const char* str_id_end, ImGuiID seed);
