...
#define SHIDWORD(l)          ((signed long)((((DWORD_PTR)(l)) >> 16) & 0xffffffff))
	...
	public:
		dat_bit_buffer(void* data, uint32_t size)
		{
			m_data = data;
			m_bitOffset = 0;
			m_maxBit = size * 8;
		...
		{
			return g_pointers->m_write_bitbuf_bool(this, integer, 1);
		}
		bool ReadBool(bool* integer)
		{
			return g_pointers->m_read_bitbuf_bool(this, integer, 1);
		}
		...
		{
			return g_pointers->m_read_bitbuf_string(this, string, bits);
		}
		bool WriteByte(uint8_t integer, int bits)
		{
			return g_pointers->m_write_bitbuf_dword(this, integer, bits);
		}
		bool ReadByte(uint8_t* integer, int bits)
		{
			uint32_t read;
			if (g_pointers->m_read_bitbuf_dword(this, &read, bits))
			{
				*integer = read;
				return true;
			}
		...
		{
			return g_pointers->m_read_bitbuf_dword(this, integer, bits);
		}
		bool WriteInt32(int32_t integer, int bits)
		{
			return g_pointers->m_write_bitbuf_int32(this, integer, bits);
		}
		...
		{
			return g_pointers->m_write_bitbuf_qword(this, integer, bits);
		}
		...
		{
			return g_pointers->m_write_bitbuf_int64(this, integer, bits);
		}
		...
		{
			return g_pointers->m_write_bitbuf_array(this, array, size, 0);
		}
		...
		{
			return g_pointers->m_read_bitbuf_array(this, array, size, 0);
		}
	...
	static_assert(sizeof(dat_bit_buffer) == 0x20);
